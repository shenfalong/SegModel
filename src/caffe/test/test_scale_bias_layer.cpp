...
class WGdLossLayerTest : public ::testing::Test
{
 ...
 protected:
  WGdLossLayerTest() : blob_bottom_0_(new Blob<Dtype>()),blob_bottom_1_(new Blob<Dtype>()), blob_top_(new Blob<Dtype>())
  {
  ...
  virtual void SetUp()
  {
    blob_bottom_0_->Reshape(5, 3, 2, 4);
    blob_bottom_1_->Reshape(5, 1, 1, 1);
    ...
    for (int i=0;i<blob_bottom_1_->count();i++)
    	blob_bottom_1_->mutable_cpu_data()[i]=i%3;		
...
TYPED_TEST(WGdLossLayerTest, TestGradient)
{
  ...
  layer_param.mutable_noise_param()->set_classes(3);
