...
template <typename Dtype>
void GuidedCRFLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top, const vector<Blob<Dtype>*>& bottom)
{
	int num = bottom[0]->num();
	int maxStates = bottom[0]->channels();
	int channels = bottom[1]->channels();
...
//---------------------------------------------------------------- 
	...
	{
		caffe_gpu_add(maxStates*nNodes,alpha,tempPot.gpu_diff(),Dtype(0),compatPot.gpu_diff(),compatPot.mutable_gpu_diff());
		caffe_gpu_add(maxStates*nNodes,Dtype(1) ,tempPot.gpu_diff(),Dtype(1),bottom_diff         ,bottom_diff);
		...
		if (this->lr_mult()[0] != 0)
		{
			LOG(FATAL)<<"should not learn in Guided CRF layer !!!!!!!!!!!!!!!";
			//caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasTrans, maxStates, maxStates,nNodes,
			//											(Dtype)1., compatPot.gpu_diff(),filterPot.gpu_data(),
	...
	caffe_gpu_add(tempPot.count(),Dtype(1),tempPot.gpu_diff(),Dtype(1),bottom[0]->gpu_diff(),bottom[0]->mutable_gpu_diff());
}
template <typename Dtype>
void GuidedCRFLayer<Dtype>::SecForward_gpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
{
}
