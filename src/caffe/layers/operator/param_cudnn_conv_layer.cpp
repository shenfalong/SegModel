...
void ParamCuDNNConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
{
...
//----------------------------------------	
  ...
  cudnn::createFilterDesc<Dtype>(&filter_desc_, num_output_, channels_, kernel_size_, kernel_size_);
...
template <typename Dtype>
void ParamCuDNNConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
{
  ...
  CHECK_EQ(bottom[1]->width(),1);
	...
	height_out_ = (height + 2 * pad_ - kernel_size_) / stride_ + 1;
  width_out_ = (width + 2 * pad_ - kernel_size_) / stride_ + 1;
...
template <typename Dtype>
ParamCuDNNConvolutionLayer<Dtype>::~ParamCuDNNConvolutionLayer() 
{
...
INSTANTIATE_CLASS(ParamCuDNNConvolutionLayer);
REGISTER_LAYER_CLASS(ParamCuDNNConvolution);
}   // namespace caffe
